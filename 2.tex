...
\begin{document}
  ...
  \begin{flushleft}
    ...
    \begin{enumerate}
      ...
      \begin{itemize}
        \item Создать задание на запуск процесса в определенное время. Например, создать задание на запуск программы \cmd{gedit} в определенное время
        \item Отобразить список заданий на выполнение по времени
        \item Изменить программу, которая должна быть выполнена в определенное время, например, вместо программы \cmd{gedit} будет выполняться программа \cmd{gnome-terminal}
        \item Удалить задание
        \item Запустить программу в отдельном окне в различных режимах, например, запустить программу \cmd{gedit} в развернутом окне и в свернутом окне
        \item Отобразить список процессов, выполняющихся на рабочей станции
        \item Отфильтровать список процессов так, чтобы в нем остались только запущенные нами программы \cmd{gedit} в предыдущем задании
        ...
        \item Завершить процесс по имени процесса, например, завершить все процессы \cmd{gedit}
      \end{itemize}
    \end{enumerate}
  \end{flushleft}
